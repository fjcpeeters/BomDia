...
% Agemodel.best_LB5_UB95 = [Z,AgemodelMC.all(:,indxcarMCstd(1)),...
%    AgemodelMC_LB5, AgemodelMC_UB95];
toc

stats = struct;

stats.car_nbest_mean = mean(sgmnt.carMCbest(5:end,:),'all');
stats.car_nbest_std =std(sgmnt.carMCbest(5:end,:),0,'all');
...
% PLOT 1 and 2
subplot(3,3,1:2)
plot(Z, CaCO3original,'Color',[0 0.4470 0.7410])
hold on
...
% title (['Carbonate accum. rate (CAR) for best ',num2str(nbest),...
%    ' out of ',num2str(nsim),' MC solutions'])
...
subplot(3,3,7:8)
% for k=2:nbest
%     plot(Z_for_SR,SRMC(:,indxcarMCstd(k)),'Color',[0 0.4470 0.7410])
%     hold on
% end
% plot(Z_for_SR,SRMC(:,indxcarMCstd(1)),'r','LineWidth', 2)
% hold off
% set(gca, 'YScale', 'log')
...
hold on
    ...
    plot(ACP_depth, ageminorig,'^k')
    plot(ACP_depth, agemaxorig,'vk')
    % plot(Z,AgemodelMC_mean,'--r','LineWidth', 2)
hold off
grid on
% axis square
% title(['Best ', num2str(nbest),' (out of ', num2str(nsim),')',...
%    ' MC age depth models',';',' median age model (black line). ',...
%    'Shading is 90% confidence interval of best solutions'])
xlabel ('Depth [cm]'), ylabel ('Age [kyr BP]')
subplot(3,1,3)
plot(Z,2.*AgemodelMC.nbeststd,'k', 'LineWidth', 2)
grid on
...
figure(5)
plot(carMCloweststd)
title(['standard deviation of car values for ', num2str(nbest),...
     ' (out of ', num2str(nsim),')',' MC age depth models'])